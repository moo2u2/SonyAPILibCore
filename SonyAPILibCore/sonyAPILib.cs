    ...
    public class APILibrary
    {
        ...
        public class SonyDevice
        {
            ...
            public string GetRemoteCommandList()
            {
                ...
                if (this.Actionlist.RegisterMode <= 3)
                {
                    ...
                    if (cmdList != "")
                    {
                        ...
                        StringReader xmlSR = new StringReader(cmdList);
                        CommandList.ReadXml(xmlSR, XmlReadMode.Auto);
                        DataTable IRCCcmd = new DataTable();
                        var items = CommandList.Tables[0].SelectSonyCommands();//.Select(r => new SonyCommands { name = r["name"), value = r["value") });
                        this.Commands = items.ToList();
            ...
            private string GetRegistrationMode()
            {
                ...
                if (ActionListUrl != "")
                {
                    ...
                    act = acList.Tables[0];
                    var results = from DataRow myRow in act.Rows where (string)myRow["name"] == lAction select myRow;
                    foundVal = (string)results.ElementAt(0)["mode"];
                }
                else
                {
            ...
            public string GetDeviceMac(SonyDevice mDev)
            {
                try
                {
                    ...
                    if (this.Actionlist.RegisterMode == 3)
                    {
                        ...
                        var siresults = from DataRow myRow in sysi.Rows where (string)myRow["field"] == "MAC" select myRow;
            ...
            public bool BuildFromDocument(string Doc, string Path)
            {
                ...
                try
                {
                    ...
                    this.ServerName = Environment.MachineName + "(SonyAPILib)";
                    ...
                    foreach (XmlNode node in xNode.ChildNodes)
                    {
                        if (node.Name == "av:X_UNR_DeviceInfo")
                        {
                            ...
                            foreach (XmlNode dserv in node.ChildNodes)
                            {
                                if (dserv.Name == "av:X_CERS_ActionList_URL")
                                {
                                    ...
                                    DataTable act = new DataTable();
                                    act = acList.Tables[0];
                                    var results = from DataRow myRow in act.Rows where (string)myRow["name"] == "register" select myRow;
                                    ...
                                    results = from DataRow myRow in act.Rows where (string)myRow["name"] == "getSystemInformation" select myRow;
                                    this.Actionlist.SystemInformationUrl = this.CheckFullPath(results.ElementAt(0).ItemArray[2].ToString());
                                    results = from DataRow myRow in act.Rows where (string)myRow["name"] == "getRemoteCommandList" select myRow;
                                    this.Actionlist.RemoteCommandListUrl = this.CheckFullPath(results.ElementAt(0).ItemArray[2].ToString());
                                    results = from DataRow myRow in act.Rows where (string)myRow["name"] == "getStatus" select myRow;
                                    this.Actionlist.StatusUrl = this.CheckFullPath(results.ElementAt(0).ItemArray[2].ToString());
                                    results = from DataRow myRow in act.Rows where (string)myRow["name"] == "getText" select myRow;
                                    this.Actionlist.GetTextUrl = this.CheckFullPath(results.ElementAt(0).ItemArray[2].ToString());
                                    results = from DataRow myRow in act.Rows where (string)myRow["name"] == "sendText" select myRow;
                        ...
                        if (node.Name == "av:X_IRCCCodeList")
                        {
                            _Log.AddMessage("Found Node: X_IRCCCodeList", false);
                            if (this.Commands == null) this.Commands = new List<SonyCommands>(node.ChildNodes.Count);
                            foreach (XmlNode cnode in node.ChildNodes)
                            {
                                this.Commands.Add(new SonyCommands { name = cnode.Attributes["command"].Value, value = cnode.InnerText });
                            }
                        }
                    }
        ...
        public static class HelperDLNA
        {
            ...
            {//Just returns a TCP socket ready to use
                ...
                {
                    _Log.AddMessage("Error creating socket", true);
                }
                return SocWeb;
            }
        ...
        public class RenderingControl1
        {
            ...
            public String ListPresets(SonyDevice parent)
            {
                if (parent.RenderingControl.ControlUrl != null)
                {
                    ...
                    string ret = "";
                    using (Socket SocWeb = HelperDLNA.MakeSocket(parent.IPAddress, parent.Port))
                    {
                        ...
                        if (GG.Contains("Forbidden"))
                        {
                            _Log.AddMessage("Error posting request", true);
                            return null;
                        }
            ...
            public Boolean GetMute(SonyDevice parent)
            {
                if (parent.RenderingControl.ControlUrl != null)
                {
                    ...
                    if (ReturnCode == 403)
                    {
                        _Log.AddMessage("Not authorised to make the request", true);
                        return false;
                    }
                    GG = Extentions.ChopOffBefore(GG, "<?xml");
                    GG = "<?xml" + GG;
            ...
            public int GetVolume(SonyDevice parent)
            {
                if (parent.RenderingControl.ControlUrl != null)
                {
                    ...
                    if (ReturnCode == 403)
                    {
                        _Log.AddMessage("Not authorized to make the request", true);
                        return -1;
                    }
                    GG = Extentions.ChopOffBefore(GG, "<?xml");
                    GG = "<?xml" + GG;
                    ...
                    foreach (XmlNode node in xNode.ChildNodes)
                    {
                        if (node.InnerText != "0")
                        {
                            ...
                            {
                                _Log.AddMessage("Error checking volume: " + node.InnerText, true);
                                ret = -1;
                            }
                        }
                    }
        ...
        public class AVTransport1
        {
            ...
            public void GetTransportInfo(SonyDevice parent)
            {
                if (parent.AVTransport.ControlUrl != null)
                {
                    ...
                    if (ReturnCode == 403)
                    {
                        _Log.AddMessage("Not authorized", true);
                        return;
                    }
                    GG = Extentions.ChopOffBefore(GG, "<?xml");
                    GG = "<?xml" + GG;
